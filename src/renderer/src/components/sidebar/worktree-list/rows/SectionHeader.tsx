...
export function renderWorktreeSectionHeaderRow(args: {
  ...
  const parsedOwner = row.repo ? parseExecutionHostId(getRepoExecutionHostId(row.repo)) : null
  const sshOwnerEnvironmentId =
    !isPairedWebClientWindow() && parsedOwner?.kind === 'runtime' ? parsedOwner.environmentId : null
