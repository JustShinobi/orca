...
export async function prepareAutomationDispatchWorkspace(args: {
  ...
  if (sshTargetId) {
    // Why: server-owned SSH targets (a paired Orca runtime) are managed through
    // runtime RPC rather than the desktop's local SSH provider. Folder
    // workspaces resolve their SSH target independently of repo ownership.
    const sshOwnerEnvironmentId =
      automationWorkspaceScope?.type === 'folder'
        ? null
        : getExplicitRuntimeOwnerEnvironmentId({ repos: [repo], settings: state.settings }, repo.id)
    if (sshOwnerEnvironmentId) {
      await hydrateRuntimeEnvironmentSshState(sshOwnerEnvironmentId)
    } else {
      const needsPrompt = await window.api.ssh.needsPassphrasePrompt({ targetId: sshTargetId })
      if (needsPrompt) {
        await markDispatchResult({
          runId: run.id,
