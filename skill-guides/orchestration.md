...
- It failed at `dispatch_input` with `recovery: buffered_not_submitted`: the prompt is buffered in the composer, but this Dispatch is already `failed` and its capability revoked — work run there cannot be reported. Do not press Enter. Clear the composer (Esc), then start a replacement with `worker-start --task <task> --retry-of <id>`. Never re-run the original dispatch — that duplicates the prompt.
